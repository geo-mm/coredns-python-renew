...
import hosts/*
reload 3
    """.replace('##DNS##', ' '.join(dns))

def create_yam_conf(args):
    dest = {
        'version': '2',
        'services': {
            'api': {
...
def generate(args):
    ...
    try:
        ...
        for k in conf.keys():
            ...
            with open('{}/Corefile'.format( c_k_path), mode='w', encoding="utf-8") as fp:
                fp.write(gen_root_corefile(conf[k]['dns']))
    ...
    docker = create_yam_conf(yaml_conf)
    docker_file = '{}/docker-compose.yaml'.format(root_path)

    with open(docker_file, mode='wb') as fp:
        yaml.dump(docker, fp)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Docker-compose generator for customized DNS service')

    subparsers = parser.add_subparsers(help='Sub-command list', dest='command')
    cmd_generate = subparsers.add_parser('generate', help='generate base configuration')
    cmd_generate.add_argument('-c','--conf', action='append', help='<Required> configuration with interface, eg: abc/0.0.0.0', required=True)
    cmd_generate.add_argument('--conf_path', help='configuration file path')
    cmd_generate.add_argument('-p', '--path',  help='destination path, default is current directory', required=False)

    cmd_create = subparsers.add_parser('create', help='create  startup/shutdown scripts')
    cmd_create.add_argument('-d', '--destination', action='append', help='<Required> destination DNS host', required=True)
    cmd_create.add_argument('-n', '--hostname', action='append', help='<Required> hostname to register', required=True)
